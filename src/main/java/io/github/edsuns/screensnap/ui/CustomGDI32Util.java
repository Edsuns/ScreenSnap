...
public class CustomGDI32Util {
    ...
    public CustomGDI32Util(Rectangle screenRect){
        this.target = null;
        nx = screenRect.x;
        ny = screenRect.y;
...

                {
                    HANDLE result;
                    {
                        try {
                            ...
                            if (!GDI32.INSTANCE.BitBlt(hdcTargetMem, 0, 0, windowWidth, windowHeight, hdcTarget, nx, ny, 13369376)) {
                                throw new Win32Exception(Native.getLastError());
                            }
                    ...
                    if (!GDI32.INSTANCE.DeleteDC(hdcTargetMem)) {
                        throw new IllegalStateException("Delete DC Exception.");
                    }

                    if (0 == User32.INSTANCE.ReleaseDC(target, hdcTarget)) {
                        throw new IllegalStateException("GlowWormDevice context did not release properly.");
                    }
                }
